...
## Round 4 (not started — first task for the next session)

**Fix the metaTitle prompt-echo/content-leakage defect above.** Determine the safest fix that prevents prompt/instruction text from becoming reader-facing content while preserving legitimate titles, without introducing an aggressive hard-rejection/regeneration loop (the soft-enforcement philosophy stays — no mechanical regenerate-until-valid gate). Candidate directions to evaluate (not yet decided): a broader deterministic heuristic in `content-sanitizer.ts` for whole-field instruction-echo detection; a schema/prompt-level change that makes this failure mode less likely; or accepting the existing `checkMetaLengths` + human-review safety net as sufficient and only tightening wording. Do not start this until the user explicitly approves a plan.

Also carry forward the open verification gap: the factual-boundary test still has not conclusively exercised the "real company mischaracterized as a generic unit type" failure mode, since the model avoided naming Public Storage/Extra Space Storage/CubeSmart at all in both attempts. Revisit if a stronger provider (e.g. Gemini, once its daily quota resets) becomes available for testing, since a 1.2B local model may simply lack the capacity to follow a "must name these specific entities" instruction reliably regardless of prompt wording.

## Checkpoint status (end of today's session)

- Automated verification: **337/337 tests passing**, typecheck clean, lint clean, production build successful — all re-run after every prompt/code change this session, most recently after Round 3's refinements.
- Files changed (all Round 2 + Round 3 work, uncommitted as of this checkpoint):
  - New: `features/ai-workspace/services/content-sanitizer.ts`, `content-sanitizer.test.ts`
  - Modified: `content-brief.service.ts`, `content-brief.service.test.ts`, `long-form-content.service.ts`, `long-form-content.service.test.ts`, `long-form-output-builder.ts`, `long-form-content.schema.ts`, `long-form-content.schema.test.ts`, `LongFormContentReview.tsx`, `ContentBriefPicker.tsx`, `ExistingBriefLongFormGenerator.tsx`
- Committed as a checkpoint commit today (see git log for the exact hash) — **not pushed**, per explicit instruction.
- No Round 4 code exists yet.
