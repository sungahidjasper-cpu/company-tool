...
describe("startInternalLinkAnalysisAction", () => {
  ...
  it("2. rejects invalid input (missing contentId) without any lookup", async () => {
    const result = await startInternalLinkAnalysisAction({ seoProjectId: "seo-1" } as never);
    expect(result.success).toBe(false);
  ...
  it("6. rejects when the source content belongs to a different SEO project than the one selected", async () => {
  ...
  it("7. creates an INTERNAL_LINK_ANALYSIS job and kicks off runAiGenerationJob, unawaited", async () => {
    const result = await startInternalLinkAnalysisAction(VALID_INPUT);
    expect(result.success).toBe(true);
    if (result.success) expect(result.data.jobId).toBe("job-1");
    expect(mockedCreateAiGenerationJob).toHaveBeenCalledWith(
      expect.objectContaining({ companyId: COMPANY_A, seoProjectId: "seo-1", contentId: "content-1", taskType: "INTERNAL_LINK_ANALYSIS" })
    );
    expect(mockedRunAiGenerationJob).toHaveBeenCalledWith("job-1");
  });

  it("8. reuses an existing active job for the identical input instead of creating a second one", async () => {
